  ...
  return (
    <header className="fixed backdrop-blur-md z-50 inset-x-0 top-0 flex items-center justify-between px-12 md:px-20 py-6">
      <NavLink to={"/"} className="flex items-center justify-center gap-4">
        <img src={logo} className="w-12" alt="logo" width={30} />
        ...
        {user ? (
          <>
            <div className="relative cursor-pointer" onMouseEnter={() => setIsMenu(true)}>
              ...
              {/* dashboard, logout , profile section  */}

              { isMenu && (
                <motion.div 
                {...slideTop}
                onMouseLeave={() => setIsMenu(false)} className="px-6 w-48 py-4 bg-lightOverlay backdrop-blur-md rounded-md shadow-md absolute top-12 right-0 flex flex-col gap-4">
                ...
                onClick={signOut} className="group flex items-center justify-center px-3 py-2 rounded-md shadow-md bg-gray-100 hover:bg-gray-200 gap-3">
          ...
          <>
            <NavLink to={"/login"}>
              <motion.button
                {...buttonClick}
                className="px-4 py-2 rounded-md shadow-md bg-lightOverlay border-red-300 cursor-pointer "
              >
                Login
              </motion.button>
            </NavLink>
